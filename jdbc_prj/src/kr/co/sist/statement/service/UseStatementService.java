...
public class UseStatementService {
	...
	public void modifyCpEmp1() throws NumberFormatException {
		...
		CpEmp1UpdateVO ceuVO = new CpEmp1UpdateVO(parseInt(csvArr[0]), parseInt(csvArr[2]), csvArr[1]);
		...
		try {
			...
			if (rowCnt != 0) {// 변경된 레코드가 있다. => 사원번호에 일치하는 레코드가 있다.
	...
	public void searchOneCpEmp1() {
		...
		if(inputData == null) {//취소버튼, x번트이 눌렸을 때 아랬줄을 실행하지 않는다.
			return;
		}//end if
